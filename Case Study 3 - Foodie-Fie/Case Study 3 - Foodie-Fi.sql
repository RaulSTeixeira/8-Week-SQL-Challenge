----------------------------------
-- CASE STUDY 3: FOODIE FIE --
----------------------------------

-- Author: Raul Teixeira
-- Date: 17/11/2023
-- Data Base used: Microsoft SQL Server
-- RDBMS used: Azure Data Studio
...
DROP TABLE foodie_fi.subscriptions;
...
-- Insert only allows up to 1000 lines, so it needs to be split
...
SELECT COUNT(DISTINCT(customer_id)) AS number_different_customers
...
SELECT 
        ...
        COUNT(sub.plan_id) as monthly_distribution
FROM foodie_fi.subscriptions sub
INNER JOIN foodie_fi.plans pl ON sub.plan_id = pl.plan_id
WHERE sub.plan_id = 0
...
SELECT pl.plan_id,
       pl.plan_name,
       COUNT(pl.plan_name) as nr_events
FROM foodie_fi.subscriptions sub
INNER JOIN foodie_fi.plans pl ON sub.plan_id = pl.plan_id
...
DECLARE @total_customers FLOAT = (SELECT COUNT(DISTINCT sub.customer_id) FROM foodie_fi.subscriptions sub)
SELECT
  COUNT(DISTINCT(sub.customer_id)) AS churn_customers,
  @total_customers AS total_customers,
  COUNT(DISTINCT(sub.customer_id)) * 100 / @total_customers  AS 'churn_percentage(%)'
FROM foodie_fi.subscriptions sub
WHERE sub.plan_id = 4
...
-- First create a temporary table (#rank_plan) to rank order of plans by customer
...
SELECT
  ...
  sub.start_date,
  pl.plan_name,
  RANK() OVER(PARTITION BY sub.customer_id ORDER BY sub.start_date) AS rank_plan
INTO #rank_plan
FROM foodie_fi.subscriptions sub
INNER jOIN foodie_fi.plans pl ON pl.plan_id = sub.plan_id

-- Calculate the total amount of customers by creating a variable
...
SELECT
COUNT(DISTINCT(rp.customer_id)) AS number_churn_straight,
@total_customers AS total_customers,
ROUND((COUNT(DISTINCT(rp.customer_id)) *100 / @total_customers),3) AS perc_churn_straight
FROM #rank_plan rp
WHERE rp.plan_id = 4 AND rp.rank_plan = 2
...
SELECT rp.plan_name,
       COUNT(rp.customer_id) AS nr_customers_after_trial,
       (COUNT(rp.customer_id) * 100 / @total_customers) AS perc_customers_after_trial

FROM #rank_plan rp
WHERE rp.rank_plan = 2
GROUP BY (rp.plan_name);

--B.7 What is the customer count and percentage breakdown of all 5 plan_name values at 2020-12-31?

-- First we create a CTE to do two things: restrict active customers until 2020-12-31 and assign to each customer the most recent plan

DECLARE @total_customers FLOAT = (SELECT COUNT(DISTINCT sub.customer_id) FROM foodie_fi.subscriptions sub WHERE sub.start_date <= '2020-12-31');

WITH rank_plan_cte AS(
  SELECT rp.customer_id,
        MAX(rp.rank_plan) as maximum_rank_plan      
  FROM #rank_plan rp
  WHERE rp.start_date <= '2020-12-31'
  GROUP BY rp.customer_id)

-- Then we inner join with the temporary table #rank_plan to group by plan

SELECT #rank_plan.plan_id,
       #rank_plan.plan_name,
       COUNT(rpc.customer_id) as customer_count,
       CAST(COUNT(rpc.customer_id) AS FLOAT) * 100.0 / @total_customers as customer_count_perc
FROM rank_plan_cte rpc
INNER JOIN #rank_plan ON rpc.customer_id = #rank_plan.customer_id
WHERE #rank_plan.rank_plan = rpc.maximum_rank_plan
GROUP BY plan_name, plan_id
ORDER BY plan_id

--B.8 How many customers have upgraded to an annual plan in 2020?

SELECT COUNT(DISTINCT sub.customer_id) as nr_customers_updg_annual_plan
FROM foodie_fi.subscriptions sub
WHERE sub.plan_id = 3 AND YEAR(sub.start_date) = 2020
...
DROP TABLE IF EXISTS #ypt_start_trial;
  SELECT
    rp.customer_id,
    CASE
      WHEN rp.plan_id = 0 THEN rp.start_date
      ELSE NULL
      END AS plan_trial_start_date
  INTO #ypt_start_trial
  FROM #rank_plan rp

DROP TABLE IF EXISTS #ypt_start_yearly;
  SELECT
    rp.customer_id,
    CASE
      WHEN rp.plan_id = 3 THEN rp.start_date
      ELSE NULL
      END AS plan_yearly_start_date
  INTO #ypt_start_yearly
  FROM #rank_plan rp

DELETE FROM #ypt_start_trial WHERE #ypt_start_trial.plan_trial_start_date IS NULL
DELETE FROM #ypt_start_yearly WHERE #ypt_start_yearly.plan_yearly_start_date IS NULL

SELECT
  AVG(DATEDIFF(day, st.plan_trial_start_date, sy.plan_yearly_start_date)) AS 'avg_time_to_upgrade(days)'
